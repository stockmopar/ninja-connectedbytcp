		...
		cbtcp.GetRoomStateByName(room["name"], function(error,state,level){
			if (!error){
				...
				if(st.on != state || currentTime > (lastUpdate + 1000*60) ){
					if(st.on != state){
						app.log.info("(TCP Lights) " + room["name"] + " has state has changed. New state is (" + state + ") with a level of " + level);
					}else{
						app.log.info("(TCP Lights) " + room["name"] + " has state has not changed. But refreshing state is (" + state + ") with a level of " + level);
					}
